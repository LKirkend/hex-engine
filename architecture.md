...
### 3. `src/evaluator.rs`
**Description**: `HexEvaluator` dual-mode evaluation: `evaluate_for_player()` blends 75% resistance-based network evaluation with 25% heuristic features (shortest-path, center dominance, bridge templates), plus bridge-chain threat detection. `evaluate_fast()` uses heuristic-only mode with bridge-chain penalties for deep leaf nodes. Features `bridge_chain_reach()` BFS through 2-bridge virtual links computing opponent chain span fraction (0.0-1.0) with exponential threat penalties for chains spanning >50% of the board axis, 0-1 BFS shortest path seeded with precomputed Edge-2/Edge-3 templates, amplified threat scoring (dist=0→800, dist=1→400), carrier interception boost in move ordering (+95 for moves disrupting opponent bridge carriers), center core dominance, and sudden-death detection.
...
### 8. `src/nic_replica.rs` & `src/bin/arena.rs`
**Description**: `NicReplicaEngine` in `src/nic_replica.rs` provides an algorithmic replica of Nintendo's Impossible Computer (NIC) from *Clubhouse Games: 51 Worldwide Classics*, using pure 1-ply greedy conductance-delta evaluation (no minimax tree search). For each legal move, computes $\Delta G_{own}$ (own conductance gain) and $\Delta G_{opp}$ (opponent conductance loss) via the resistance network, scoring as $\Delta G_{own} \times 100 + \Delta G_{opp} \times 80 + \text{center\_bias} + \text{tactical\_bonuses}$. Includes compulsory carrier defense, immediate win detection, and must-block opponent win. Nash Engine achieves **90.0% win rate (9/10)** against authentic NIC replica at depth 6 (**100.0% as Red (5/5), 80.0% as Blue (4/5)**, avg 7.25s/game). `src/bin/arena.rs` is an automated tournament CLI runner; `src/bin/nic_verify.rs` verifies NIC replica fidelity against real game transcripts.

### 9. `cpp/include/hex_engine.hpp`, `cpp/src/main.cpp`, `cpp/src/benchmark.cpp`
**Description**: Modern C++20 header wrapper (`hex::HexEngine`, `hex::HexBoard`, `hex::NicEngine`), CMake configuration, standalone C++ solver binary (`hex_cpp_cli`), and standalone benchmarking suite (`hex_benchmark`) measuring throughput (NPS), latency, and resident memory (RSS).

### 10. `cpp/include/hex_gui.hpp`, `gui_move_tree.hpp`, `gui_context_menu.hpp`, `gui_renderer.hpp`, `gui_panel.hpp`, `gui_modal.hpp`, `gui_top_bar.hpp`, `cpp/src/gui_main.cpp`
**Description**: Native hardware-accelerated SDL3 GUI featuring flat-topped diamond hexagon canvas, geometry-hugging borders with 50/50 split acute corner border crests at **K1** and **A11** (indicating both players can complete through apex vertices), 4-sided coordinate labels, chess-style evaluation meter with amplified non-linear scaling ($\tanh$, reaching 2/3rds bar height at $\pm 3.0$ eval) and in-bar evaluation number riding the division line snapped to the winning color's edge, candidate move ghost previews, opening book indicators with negative book cutouts, top bar toolbar with Board Size & Search Depth incrementing number fields, "Swap Turn" toolbar button with confirmation modal to swap starting colors and reset, modal resize confirmation dialog, interactive chess-viewer style MoveTree (`MoveTree`, `MoveNode`, `MoveTokenLayout`) recording search depth (`[%depth]`), calculation elapsed time (`[%emt]`), and evaluation (`[%eval]`) per move in exported PGN strings, right-side tab Engine Model Selection dropdown (`Main Engine` vs `NIC Replica`), floating `ContextMenu` dropdown popup on right-click ("Delete" / "Delete Branch" vs "Make Primary Branch"), Left/Right arrow key cycling between moves, clickable move tokens with darker text for branching variations, zero-latency instant position cache and candidate move switching on clicks/undo/redo, and clipboard PGN import/export with starting color preservation (`[First "Red"]` / `[First "Blue"]`).
